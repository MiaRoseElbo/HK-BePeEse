...
def rename_files(directory):
    ...
    files = [file for file in files if file.startswith('CAL ') and file.endswith('.png')]

    for file in files:
        # Use regex to extract the number from the original filename
        match = re.search(r'CAL (\d+)', file)
        if match:
...
directory = '/Users/miaelbo/HK/HK-BePeEse/public/images/cal'
